...
class MainWindow(QMainWindow):
    ...
    def _setup_sidebar_layout(self): # Modified
        self.sidebar_layout.addWidget(self._create_sidebar_header("Configuration:"))
        ...
        for idx, arch_name in enumerate(supported_architectures):
            ...
            tile_label_w = 110 
            if arch_name == "AMX":
                print(f"  Adding AMX parameters to tab '{arch_name}'") # DEBUG
    ...
    def _setup_sidebar_dynamic_elements(self): # No changes
    ...
    def _create_sidebar_header(self, text, bold=True, color=None): # No changes
    ...
    def _connect_signals(self): # No changes
    ...
    def on_apply_config(self): # No changes
        try:
    ...
    def on_next_step(self): # No changes
        if self.simulator.step_forward(): self.update_gui()

    def on_prev_step(self): # No changes
